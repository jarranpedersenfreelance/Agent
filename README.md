...
| **Execution Logic** | Copies of `src/secondary/` files. Serves as secondary operating logic, the execution layer. Agent can directly edit, but changes will be overwritten on the next deploy unless submitted as part of an edit proposal that is implemented. | Read / Write / Execute |
...
| **Temp Data** | These are files in the `data/` directory, but they have no corresponding files in `src/data/`. These files are not guaranteed to persist across deployments, and may be used for general purposes. | Read / Write |
